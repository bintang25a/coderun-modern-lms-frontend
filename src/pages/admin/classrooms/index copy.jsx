...
const AddDeleteMaterials = (param) => {
  const {
    isActive,
    onClose,
    isLoading,
  ...
  const handleSubmit = async () => {
    if (selectedMaterials.length === 0) return alert("Pilih mahasiswa dulu!");
    isLoading(true);
    try {
      const promises = selectedMaterials.map((id) =>
        !isDelete
          ? createClassMaterial({ class_code, id })
          : deleteClassMaterial(class_code, id)
      );
      await Promise.all(promises);
  ...
  const availableMaterials = material?.filter(
    (u) => !selectedMaterials.includes(u.id)
  );
  const chosenMaterials = material?.filter((u) =>
    selectedMaterials.includes(u.id)
  );
  ...
  return (
    <div className={`action-form-overlay ${isActive ? `` : `inactive`}`}>
      <div className="action-form wide">
        ...
        <div className="input-container">
          <div className="input-field">
            <label>Available Students ({availableMaterials?.length})</label>
            <div className="student-box">
              {availableMaterials?.map((material) => (
                <div
                  key={material.id}
                  className="student-item"
                  onClick={() => moveToRight(material.id)}
                >
                  <span>
          ...
          <div className="input-field">
            <label>Selected to Class ({chosenMaterials?.length})</label>
            <div className="student-box selected">
              {chosenMaterials?.map((material) => (
                <div
                  key={material.id}
                  className="student-item"
                  onClick={() => moveToLeft(material.id)}
                >
                  <span>
                    {material.id} - {material.name}
                  </span>
...
export default function Classrooms() {
  ...
  return (
    <main className="admin-users">
      ...
      {addMaterials ? (
        <AddDeleteStudents
          isActive={addMaterials}
          onClose={handleAddMaterials}
          isLoading={switchLoading}
          allertSetting={setAllertSetting}
          fetchData={fetchData}
          materials={materials?.filter(
            (material) =>
              !data
                ...
                ?.material?.some(
                  (student) =>
                    student.material_number === material.material_number
                )
          )}
          class_code={selectedIds[0]}
        />
